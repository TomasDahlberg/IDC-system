...
/* 91-03-16   /   started                                                 */
/* 91-09-15   /   updated                                                 */
/* 92-10-01   /   in case 4: get_blk() reads as much as possible direct   */
/**************************************************************************/
...
unsigned char *rblk;
  ...
  sync= check= ready= 0;
  for (i=0; i<270 /* 120 */; i++)   /* read at most 120 bytes for one block */
    {
    chi= get_ch();
    switch (chi)
      {
...
static int out_seq_nr_THIS;
int outSeqNr()
{
  return ++out_seq_nr_THIS;
}
...
/*  static out_seq_nr;    */
  ...
  chksum= 0;
  wblk.seqnr= ++out_seq_nr_THIS;              /* sequence number */
