...
must be called before processVisionComCall()
...
/*
!	main loop from visionCom.c extracted to only this process routine
*/
int processVisionComCall(message, outbuf)
char *message;
char *outbuf;
{
...
port, DEBUG, maxPcs, telephoneNumber
mapPcs, telephonePC, useVars
...
*/      
	...
	enterCritical(&sysVars->lockAlarm);

	alarmMarkPtr[PCno & 7] = serial2Ptr(alarmMarkPtr[PCno & 7]);
#endif

	if (alarmMarkPtr[PCno & 7] == -1) {
#ifdef NEW_ALARM_TEST
              fprintf(fpOut, "ACK-error, WHAT ???\n");
#endif
	} else {
	...
	exitCritical(&sysVars->lockAlarm);
