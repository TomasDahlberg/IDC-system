...
char *argv[];
{
  ...
  lock = (struct _lockModule *) linkDataModule("LOCK", &headerPtr2);
  if (!lock) {
    printf("LOCK ?\n");
#ifdef VERY_MUCH_EPROM
    printf("cannot link to datamodule '%s'\n", "LOCK");
  ...
  lock2 = (struct _lockModule2 *) 
            (((char *) lock) + (lock->noOfAlarms * sizeof(int) + sizeof(int)));
  ...
  aldm = (struct _alarmModule *) linkDataModule("ALARM", &headerPtr4);
  if (!aldm) {
    printf("ALARM ?\n");
#ifdef VERY_MUCH_EPROM
    printf("cannot link to datamodule '%s'\n", "ALARM");
