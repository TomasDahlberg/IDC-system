...
void insert(unsigned char *word, int antal)
{
  WORD *tmp, *link;
...
main(int argc, char *argv[])
{
  ...
  if ((fp = fopen(argv[1], "r")) == NULL) {
    printf("cannot open sentence file '%s'\n", argv[1]);
    exit(1);
  }
  while (fgets(buff2, 256, fp)) {
    int antal;

    sscanf(buff2, "%3d, ", &antal);
/*
    fscanf(fp, "%3d, '%s'\n", &antal, buf);
*/
    strcpy(buf, &buff2[6]);
