...
void main()
  ...
  do
    {
{
  static unsigned char *l = 0x308002;
  *l = 0xff;  
}
    while (_gs_rdy(0) <= 0)
	;
    ...
    stat= get_blk(rblk);
{
  ...
  *l = 0xfe;  
	...
	as_blk(PNAK, strcat(st,perrtab[-stat])); break;
	  ...
	  break;
        case REC_CMPRS_DATA:  /* receive data */
          switch (rblk[0])
            {
  	    char utbuf[512];
 	    int utlen;
	    case PDATA:
		utlen = unpack(&rblk[1], length - 1, utbuf);

          if (fwrite(utbuf, 1, utlen, mout)== (length-1))
              send_ack();
	      break;
	    case PEOF:
	      fclose(mout);
	...
	case STOP:
	  if (rblk[0] != PQUIT)
	      ...
	      {
char b[128];
sprintf(b, "RECMOD: no error, cnt1 = %d, cnt2 = %d", cnt1, cnt2);
	      as_blk(PACK, b);
/*
"RECMOD: No unrecoverable errors! RECMOD terminating!");
*/
	      ready= 1;
	      }
	}  /* end switch (state) */
