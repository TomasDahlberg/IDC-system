...
char VERSION[] = "Getvar version 1.00";  /* versionstring */
...
void parse(unsigned char *str)
{
  unsigned char nodeNr[5], indexNr[5], varValue[32], buf[80];
  unsigned char *p;
  int ind, t;

  while( 1 ) {
    copyUntil(nodeNr, &str, ' ');
    copyUntil(indexNr, &str, ' ');
    copyUntil(varValue, &str, ' ');
    skipWhiteSpace(&str);
    ind = atoi(indexNr);
    if( ind > -1 ) {
      p = (unsigned char *) strrchr(varValue, '\r');
      if( p != NULL )
	strcpy(p, "");
//      sprintf(buf,"%s %s = %s\n", nodeNr, var[ind], varValue);
      sprintf(buf,"%s = %s\n", var[ind], varValue);
      if( DEBUG ) printf("%s", buf);
      fputs(buf, fpWritefile);
    } else {
      t   = atoi(indexNr);
      ind = abs(t);
      printf("'%s' is missing in node %s\n", var[ind], nodeNr);
    }
    str--;
    if( *str == CR || *str == NULL)
      break;
...
void main(int argc, char *argv[])
{
  ...
  if( (fpWritefile = fopen(writefile, "w") ) == NULL ) {
    printf("cannot create file '%s'", writefile);
    pexit(0);
  }
  ...
  while( 1 ) {
    strcpy(outputString, "" );               // nollst„ll str„ng
    for(i=0; i<6; i++) {
      if( fgets(str[0], 80, fpReadfile) ) {
	skipWhiteSpace(&str);
	copyUntil(buf, &str, LF);
	strcpy(var[i], buf);
	sprintf(buf, "0 %d %s", i, var[i]);
	strcat(outputString, buf);
	if( i < 5 )
	  strcat(outputString, " & ");
      } else {
	p = (unsigned char *) strrchr(outputString, '&');
	if( p != NULL ) {
	  p--;
	  strcpy(p, "\0");
	}
	writeString(outputString);
	if( DEBUG ) printf("outputString =%s\n", outputString);
	cnt = 0;
...
int readInputString()
{
  ...
  strcpy(inputString, "");           // nollst„ll str„ng
